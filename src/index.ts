...
export const name = 'idiom-solitaire'
...
let globalIdiom: string;//定义全局变量：当前正在接龙的成语


export function apply(ctx: Context, config: Config) {

  ctx.command('成语接龙 <message:string>', '发送 成语接龙 开始游戏，后面可以带一个成语来指定以什么开始').alias('/成语接龙').action(
    async (_, message) => {
      ...
      if(message == undefined){//未指定成语，机器人随机获取一个成语来开始
        ...
        if (!match_lyrics?.length) return `接口异常，请联系开发者`
        ...
        if (idiom.length !== 4)return "请发送[成语接龙 四字成语]或直接发[成语接龙]来开始游戏";//消息不是四字，提示
        else{
          if(idiom == "为所欲为"){globalIdiom = "为所欲为";return "为所欲为";}
          if(idiom == "一个顶俩"){globalIdiom = undefined;return "你杀死了比赛。。";}
  ...
  ctx.middleware(async (session, next) => {
    ...
    if(globalIdiom !== undefined){
      ...
      else return "当前接龙最后一个字是："+globalIdiom[3]+",你接的不对哦";
    }
    else {
      if(idiom == "为所欲为"){globalIdiom = "为所欲为";return "为所欲为";}
      if(idiom == "一个顶俩")return "你杀死了比赛。。";
...
function getRandomElement(arr: string[]): string | string {  
  if (arr.length === 0) {  
    return '';  
  }  
...
function isFourChineseCharsWithFirstChar(text: string, specifiedChar: string): boolean {  
  // 检查指定字符是否为中文字符  
